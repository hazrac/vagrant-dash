class packages::graphite-whisper {
  ...
  exec { "unpack-whisper":
    ...
    command => "cd $graphite-params::build_dir ; tar -zxvf $graphite-params::whisper_dl_loc ; cd whisper-0.9.9 ;",
  ...
  exec { "install-whisper":
    # true is needed to work around a problem with execs and built ins. https://projects.puppetlabs.com/issues/4884
    command => "cd $graphite-params::build_dir/whisper-0.9.9 && python setup.py install",
